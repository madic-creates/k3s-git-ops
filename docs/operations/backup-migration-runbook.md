...
Confirm rest-server is healthy:

```bash
kubectl run -n backup --rm -it --restart=Never \
  --image=curlimages/curl curltest -- \
  curl -u 'backup:hXBYuF33KCrJyfVJUthSUmzWhZCZDLEo' \
  http://rest-server:8000/
# Expected: empty 200 response (or "Restic REST Server" body)
```
...
export RESTIC_REPOSITORY='rest:http://backup:hXBYuF33KCrJyfVJUthSUmzWhZCZDLEo@127.0.0.1:8000/'
export RESTIC_PASSWORD='9C6Y1KUXUh914FDh3hBM0l5JqogjbM3yV5wOuPn2wFKi9YXMWB6aueDTOJSJVe7E'
...
export AWS_ACCESS_KEY_ID='MYGBS7C5A4SJXXLP6PTW'
export AWS_SECRET_ACCESS_KEY='oGjyM7eDXG8F8aVHR2tnj14rNzkh7gD5QlhI83yw'

# Source: Wasabi
export RESTIC_FROM_REPOSITORY='s3:s3.eu-central-2.wasabisys.com/k3s-at-home-01/restic-backup'
export RESTIC_FROM_PASSWORD='9C6Y1KUXUh914FDh3hBM0l5JqogjbM3yV5wOuPn2wFKi9YXMWB6aueDTOJSJVe7E'

# Target: local rest-server (already in $RESTIC_REPOSITORY/$RESTIC_PASSWORD from step 3)
