...
To allow [Renovate](updates.md) to update image tags and other non-sensitive values, the values can be split into two files:
...
The ConfigManagementPlugin is configured as a configmap within a separate [argo-cd app deployment](https://github.com/madic-creates/k3s-git-ops/tree/main/apps/argo-cd-configmanagementplugins){target=_blank}. Additionally it needs helm and sops binaries for the argo-cd repo-server which get configured via a sidecar container. So the deployment of it needs to be extended.
